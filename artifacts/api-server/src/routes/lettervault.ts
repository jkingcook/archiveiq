...
function getMediaType(filename: string, buffer: Buffer): "image/jpeg" | "image/png" | "image/gif" | "image/webp" {
  ...
  const doc = new Document({
    sections: [{
      children: [
        ...
        addText(`Date Range: ${dateRange} | Items: ${itemCount}`, true),
  ...
  const doc = new Document({
    sections: [{
      children: [
        ...
        ...groups.flatMap(g => [
          addText(`${g.group_label}: ${g.archivist_summary || ""}`, false),
...
router.post("/lettervault/process", upload.array("files", 50), async (req: Request, res: Response) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      mediaType = getMediaType(file.originalname, file.buffer);
    } catch (err) {
      results.push({
        item_id: itemId,
        filename: file.originalname,
